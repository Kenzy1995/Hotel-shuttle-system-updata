...
const isTripWithinStartWindow = (t?: Trip | null): boolean => {
  ...
  try {
    ...
    // 發車前30分鐘 <= 現在時間 <= 發車後1小時
    return (now >= (tripTime - BEFORE_30_MIN)) && (now <= (tripTime + AFTER_1_HOUR));
  } catch (e) {
    return false;
  }
};
                ...
                <div style={{display:'flex', flexDirection:'column', minHeight:'60vh'}}>
                  <div style={{background:'transparent', border:'none', padding:'24px', display:'flex', flexDirection:'column', justifyContent:'space-between', minHeight:'60vh'}}>
                    ...
                    <div style={{marginTop:'24px'}}>
                      {isTripWithinStartWindow(currentTrip) ? (
                        <button className="action-btn action-btn--start large" style={{width:'100%'}} onClick={() => setStartConfirm(true)}>出車開始</button>
                      ) : (
                        <button className="action-btn action-btn--start large" style={{width:'100%', opacity:0.5, cursor:'not-allowed'}} disabled>出車開始</button>
                      )}
                      {!isTripWithinStartWindow(currentTrip) && (
                        <div style={{marginTop:'8px', fontSize:14, color:'#999', textAlign:'center'}}>
                          只能在發車前30分鐘到發車後1小時內開始出車
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <button className="modal-btn primary" onClick={async () => {
                   ...
                   if (userRole !== 'desk' && gpsSystemEnabled) {
                     try {
                       ...
                       setToastMessage('無法檢查GPS狀態，請確認已開啟定位功能');
                       setStartConfirm(false);
                       return;
                     }
                   }
